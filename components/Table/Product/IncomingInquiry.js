      ...
      return (
        <button
          ...
          className={`
                        ${
                          !metaData.prevPage
                            ? 'border border-solid border-blueGray-400'
                            : 'border border-solid border-blueGray-500 text-blueGray-700'
                        }
                        text-xs font-semibold flex w-8 h-8 mx-1 p-0 items-center justify-center leading-tight relative`}
        >
          <i
            className={`${
              !metaData.prevPage ? 'text-blueGray-400' : 'text-blueGray-700'
            } fas fa-angle-left my-auto mx-10 fa-xl`}
          ></i>
        </button>
      )
    }
    ...
    if (item.label === 'Next &raquo;') {
      return (
        ...
        >
          <i
            className={`${
              !metaData.nextPage ? 'text-blueGray-400' : 'text-blueGray-700'
            } fas fa-angle-right my-auto mx-10 fa-xl`}
          ></i>
        </button>
      )
    }
    ...
    <>
      <div className="relative">
        ...
        <div className="relative overflow-x-auto ">
          <table
            className={`w-full text-sm text-left text-gray-500 shadow-md ${props.customClass}`}
          >
              ...
              {data.map((item, index) => {
                return (
                  ...
                  >
                    ...
                    <td className="text-sm px-6 py-4">{item.package}</td>
                    ...
                    <td className="text-sm px-6 py-4 text-right">
                      <div className="inline-flex">
                        <Link
                          ...
                          className="mr-2 font-medium text-blue-600 text-white bg-blueGray-700 p-2"
                        >
                          View
                        </Link>
          ...
          {metaData.total > 0 ? (
            <div className="mt-2">
              <h2>
                Showing{' '}
                {metaData.total <= 20 ? metaData.total : metaData.perPage} data
                from {metaData.total} data
              </h2>
            </div>
          ) : null}
        </div>
