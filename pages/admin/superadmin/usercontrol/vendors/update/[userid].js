...
export default function EditContributor({session, routerParam}) {    
    ...
    const loadData = async () =>{
        setIsLoading(true)
        const response = await axios.get(`/admin/vendors?id=${routerParam.userid}`,
        {
            headers: {
    ...
    const handleSubmit = async (e) => {
        ...
        const response = await axios.post(`/admin/vendors/update`, inputData, {
            headers: {
                "Authorization" : `Bearer ${session.accessToken}`
            }
        })
        .then(() => {
            ...
            toast.success("Your Vendors has been updated succefully", toastOptions)
        }).catch((error) => {
            toast.error("Something went wrong", toastOptions)
