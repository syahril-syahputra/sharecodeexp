...
export default function EditContributor({session, routerParam}) {
  ...
  const loadData = async () => {
    ...
    const response = await axios
      .get(`/admin/contributors?id=${routerParam.userid}`, {
        headers: {
      ...
      .then((response) => {
        ...
        setInputData({
          ...
          role_status: result.status_id,
          status: {
            value: `${result.status.id}`,
  ...
  const handleSubmit = async (e) => {
    ...
    const {status, ...newRosponse} = inputData;
    const response = await axios
      .post(`/admin/contributors/update`, newRosponse, {
        headers: {
          Authorization: `Bearer ${session.accessToken}`,
        },
      })
      .then(() => {
        ...
        toast.success(
          'Your contributor have been updated successfully.',
          toastOptions
        );
      })
      .catch((error) => {
