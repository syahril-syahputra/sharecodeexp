...
export default function EditContributor({session, routerParam}) {    
    ...
    const loadData = async () =>{
        setIsLoading(true)
        const response = await axios.get(`/admin/contributors?id=${routerParam.userid}`,
        {
            headers: {
    ...
    const handleSubmit = async (e) => {
        ...
        const response = await axios.post(`/admin/contributors/update`, inputData, {
            headers: {
                "Authorization" : `Bearer ${session.accessToken}`
            }
        })
        .then(() => {
            ...
            toast.success("Your contributors has been created succefully", toastOptions)
        }).catch((error) => {
            toast.error("Something went wrong", toastOptions)
