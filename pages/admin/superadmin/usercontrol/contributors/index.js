...
export default function Contributors({session}) {
  ...
  const loadData = async (srch, page = 1) => {
    ...
    const response = await axios
      .get(`/admin/contributors?page=${page}`, {
        headers: {
  ...
  const [showDeleteContributorModal, setShowDeleteContributorModal] =
    useState(false);
  ...
  const handleDeleteSelectedContributor = async (id) => {
    setIsLoading(true);
    const response = await axios
      .delete(`/admin/contributors/delete?id=${id}`, {
        headers: {
          Authorization: `Bearer ${session.accessToken}`,
        },
      })
      .then(() => {
        loadData();
        toast.success(
          'Your contributor have been deleted successfully.',
          toastOptions
        );
      ...
      .catch((error) => {
        toast.error(
          'Something went wrong. Cannot delete subscriber.',
          toastOptions
        );
      })
      .finally(() => {
