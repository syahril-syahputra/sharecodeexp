...
export default function ReturnHandlingActive({ session, routerParam }) {
  ...
  const loadOrderStatusOption = async () => {
    await axios
      .get(`/allstatus?is_closed=0&reimbursement=1`)
      .then((response) => {
  ...
  return (
    <div className="mb-10">
      <h1 className="font-semibold text-2xl">Return Handling Closed</h1>
      <PrimaryWrapper className={`mt-5 p-5`}>
        <h2 className="text-xl text-center">Search Return Handling</h2>
...
ReturnHandlingActive.layout = Admin
