...
export default function EditComponent({ session, routeParam, packaginglist }) {
  ...
  const getData = async () => {
    setIsLoading(true)
    const response = await axios
      .get(`/admin/product?id=${routeParam.componentid}`, {
        headers: {
          Authorization: `Bearer ${session.accessToken}`,
        },
      })
      .then((response) => {
        ...
        setCategory({
          value: result.subcategory.category.id,
          label: result.subcategory.category.name,
        })
        setSubCategory({
          value: result.subcategory.id,
  ...
  return (
    <PrimaryWrapper>
      <PageHeader
        ...
        rightTop={
          <Link
            href={`/admin/superadmin/components/details/${routeParam.componentid}`}
          >
      ...
      <div className="w-full mb-6">
        <div className="px-3 md:mb-0 text-center">
          <div className="p-24 border mx-2 my-4">
            product image {routeParam.componentid}
          </div>
        </div>
      </div>
