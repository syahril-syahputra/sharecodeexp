...
import ComponentList from "@/components/Table/Member/Seller/RejectedOrders"
...
export default function InquiryNow({session}) {
    ...
    const searchData = async (searchParam='', page = 1) =>{
        ...
        setIsLoading(true)
        const response = await axios.get(`/buyer/order/order_returned?page=${page}&search=${searchParam}`,
            {
            headers: {
    ...
    return (
        <>
            <div className="mb-10">
                ...
                <ComponentList
                    title="Rejected Buying Products"
                    setPage={setPage}
                    isLoading={isLoading}
                    data={data}
                    links={links}
