...
import ComponentList from "@/components/Table/Member/Seller/CompletedOrders"
...
export default function InquiryNow({session}) {
    ...
    const searchData = async (searchParam='', page = 1) =>{
        ...
        setIsLoading(true)
        const response = await axios.get(`/buyer/order/order_completed?page=${page}&search=${searchParam}`,
            {
            headers: {
    ...
    return (
        <>
            <div className="mb-10">
                ...
                <ComponentList
                    title="Completed Buying Products"
                    setPage={setPage}
                    isLoading={isLoading}
                    data={data}
                    links={links}
