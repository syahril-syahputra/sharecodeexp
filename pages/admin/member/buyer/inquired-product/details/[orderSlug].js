...
export default function InquiryDetails({session, routeParam}) {
    ...
    const loadData = async () =>{
        setIsLoading(true)
        setErrorInfo({})
        const response = await axios.get(`/buyer/order/${routeParam.orderSlug}/detail`,
            {
            headers: {
    ...
    const acceptQuotationModalHandle = async () => {
        setIsLoading(true)
        const response = await axios.post(`/buyer/order/accept-quotation`, 
            {
    ...
    const rejectQuotationModalHandle = async (quotationRejectionReason) => {
        setIsLoading(true)
        const response = await axios.post(`/buyer/order/reject-quotation`, 
        {
            'order_slug':  data.slug,
    ...
    const sendPaymentDocsModalHandle = async (paymentDocument) => {    
        ...
        const response = await axios.post(`/buyer/order/pay-order`, 
            formData,
        {
            headers: {
                "Authorization" : `Bearer ${session.accessToken}`
    ...
    const sendUpdatedPaymentDocsModalHandle = async (paymentDocument) => { 
        ...
        formData.append("buyer_receipt", paymentDocument);
        formData.append("order_slug", data.slug)        
        const response = await axios.post(`/buyer/order/update-payment`, 
            formData,
        {
            headers: {
                "Authorization" : `Bearer ${session.accessToken}`
    ...
    const acceptOrderModalHandle = async () => {
        setIsLoading(true)
        const response = await axios.post(`/buyer/order/accept-order`, 
        {
            order_slug:  data.slug
        },
        {
    ...
    const handleDidntReceiveAny = async () => {
        setIsLoading(true)
        const response = await axios.post(`/buyer/order/did-not-receive`, 
        {
            order_slug:  data.slug
        },
        {
